...
<form method="post" action="<?= Router::url('/forgot-password') ?>" class="form-stack auth-form">
    ...
    <div class="auth-field">
        ...
        <input class="input auth-input-field" id="forgot-email" type="email" name="email" required autocomplete="email">
    </div>
    <button class="btn btn-primary btn-block auth-submit" type="submit"><?= Lang::e('auth.forgot_submit') ?></button>
</form>
<p class="auth-foot"><a href="<?= Router::url('/login') ?>"><?= Lang::e('auth.back_login') ?></a></p>
