...
final class UserCar
{
    ...
    {
        $stmt = Database::prepare('SELECT car_id FROM user_cars WHERE user_id = ? ORDER BY car_id');
        $stmt->execute([$userId]);
        $out = [];
    ...
    public static function quotaForUserCar(int $userId, int $carId): float
    {
        ...
        $stmt = Database::prepare('SELECT quota_percent FROM user_cars WHERE user_id = ? AND car_id = ? LIMIT 1');
    ...
    private static function filterValidCarIds(array $carIds): array
    {
        ...
        $stmt = Database::prepare("SELECT id FROM cars WHERE id IN ($ph)");
        $stmt->execute($carIds);
        $out = [];
        foreach ($stmt->fetchAll(PDO::FETCH_COLUMN) as $id) {
            $out[(int) $id] = true;
    ...
    public static function revenueByMonth(int $userId, int $months = 6): array
    {
        ...
        for ($i = $months - 1; $i >= 0; $i--) {
            ...
            $start = date('Y-m-01', $monthTs);
            $endTs = strtotime($start);
            if ($endTs === false) {
                continue;
            }
            $end = date('Y-m-t', $endTs);
            $label = date('Y-m', $endTs);
            $sql = "SELECT COALESCE(SUM(r.final_amount * ({$quotaExpr} / 100)), 0)
                    FROM user_cars uc
                    JOIN reservations r ON r.car_id = uc.car_id
                    WHERE uc.user_id = ?
