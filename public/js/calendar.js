(() => {
  ...
  function setLoading(on) {
    ...
    if (loading) {
      loading.classList.toggle('is-busy', on);
      loading.classList.toggle('is-idle', !on);
    }
  }
  ...
  async function loadEvents() {
    ...
    try {
      ...
      root.innerHTML = `<div class="cal-error" role="alert"><p>${escapeHtml(window.__calError || 'Erro ao carregar calendário.')}</p><button type="button" class="btn btn-secondary btn-sm" id="calRetry">${escapeHtml(window.__calRetry || 'Tentar novamente')}</button></div>`;
      document.getElementById('calRetry')?.addEventListener('click', render);
      return [];
    } finally {
      setLoading(false);
    }
